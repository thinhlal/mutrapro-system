...
SELECT gen_random_uuid(), 'transcription', 'per_minute', 50000.00, 'VND', 'Price per minute', true, 'SYSTEM', NOW(), 'SYSTEM', NOW()
WHERE NOT EXISTS (SELECT 1 FROM pricing_matrix WHERE service_type = 'transcription');

-- Arrangement: tính theo bài
INSERT INTO pricing_matrix (pricing_id, service_type, unit_type, base_price, currency, description, is_active, created_by, created_at, updated_by, updated_at)
SELECT gen_random_uuid(), 'arrangement', 'per_song', 1500000.00, 'VND', 'Price per song', true, 'SYSTEM', NOW(), 'SYSTEM', NOW()
...
INSERT INTO pricing_matrix (pricing_id, service_type, unit_type, base_price, currency, description, is_active, created_by, created_at, updated_by, updated_at)
SELECT gen_random_uuid(), 'arrangement_with_recording', 'per_song', 2000000.00, 'VND', 'Price for arrangement and recording per song.', true, 'SYSTEM', NOW(), 'SYSTEM', NOW()
