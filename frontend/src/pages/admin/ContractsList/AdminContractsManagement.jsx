...
export default function AdminContractsManagement() {
  ...
  const buildFilterParams = (page = pagination.current - 1, pageSize = pagination.pageSize) => {
    const filters = {};
    if (search) filters.search = search;
    if (type) filters.contractType = type;
    if (status) filters.status = status;
    if (currency) filters.currency = currency;
    filters.page = page; // Backend uses 0-indexed pages
    filters.size = pageSize;
    return filters;
  };

  // Fetch contracts từ API với filter và pagination
  const fetchContracts = async (page = pagination.current - 1, pageSize = pagination.pageSize) => {
    try {
      setLoading(true);
      setError(null);
      const filters = buildFilterParams(page, pageSize);
      ...
      if (response?.data?.content) {
        ...
        setPagination({
          current: response.data.pageNumber + 1, // Convert 0-indexed to 1-indexed
          pageSize: response.data.pageSize,
          total: response.data.totalElements,
        });
      } else if (Array.isArray(response?.data)) {
        // Fallback: List response format (backward compatibility)
  ...
  useEffect(() => {
    fetchContracts(0, 10); // Reset to first page when filters change
  ...
  // Data đã được filter và paginated ở backend, chỉ cần dùng trực tiếp
      ...
      <div className={styles.toolbar}>
        ...
        <Button
          ...
          onClick={async () => {
            ...
            setStatus();
            setCurrency();
            // Reload data (reset về page 1)
            fetchContracts(0, pagination.pageSize);
