 ...
 * @param {string} params.reviewType - Filter by review type (TASK, REQUEST, PARTICIPANT)
 * @param {string} params.specialistId - Filter by specialist ID
 * @param {string} params.requestId - Filter by request ID
 * @param {number} params.minRating - Filter by minimum rating (1-5)
 * @param {number} params.maxRating - Filter by maximum rating (1-5)
...
export const getAllReviews = async (params = {}) => {
  try {
    const response = await axiosInstance.get(
      API_ENDPOINTS.REVIEWS.GET_ALL_REVIEWS,
      { params }
    );
    return response.data;
  } catch (error) {
